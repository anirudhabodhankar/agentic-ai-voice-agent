# credit to  https://github.com/ReallyEasyAI/Working-with-the-Audio-APIs/blob/main/Text_to_Speech.ipynb
import threading  
import queue  
import re  
import os
...
class TextToGPTAudioStreamGenerator:  
    ...
    def az_speech_synthesis_callback(self, evt):
        ...
        if evt.result.reason == speechsdk.ResultReason.SynthesizingAudio:
            ...
            # if(self.sentence_generation_complete.is_set()):
            #     console_logger.info(f'Adding audio chunk to queue of size: {len(audio_chunk)}')
    ...
    def generate_sentences(self) -> None:  
        ...
        sentence_pattern: re.Pattern = re.compile(r'[^.!।?:\n\t]+[.!।?:\n\t]')  
    ...
    def generate_audio_gpt(self, llm_audio_generator: Runnable) -> None:  
        """  
        ...
        while not (self.sentence_generation_complete.is_set() and self.sentence_queue.empty()):  
            try:  
                ...
                with llm_audio_generator.audio.speech.with_streaming_response.create(  
                    model="tts-1",  
                    voice="onyx",  
                    input=sentence,  
                    response_format="pcm",  
                    ...
                    for audio_chunk in tts_response.iter_bytes(1024):  
    ...
    async def audio_queue_iterator(self) -> AsyncGenerator[bytes, None]:  
        ...
        # console_logger.info(f'Inner - Total audio chunks generated: {self.total_audio_chunks}')
        # console_logger.info(f'Inner - Total audio chunks on queue_iter: {self.total_audio_chunks_on_queue_iter}')
        # self.audio_queue_iterator_complete.set()
    ...
    async def generate_audio_chunks(  
        ...
        Args:  
            llm_audio_generator (Runnable): Runnable instance for audio generation.  
        ...
        with console_tracer.start_as_current_span("generate_audio_chunks") as span:
            ...
            #audio_gen_thread: threading.Thread = threading.Thread(target=self.generate_audio, args=(llm_audio_generator,), daemon=True)  
            ...
            # while not self.audio_queue_iterator_complete.is_set():
