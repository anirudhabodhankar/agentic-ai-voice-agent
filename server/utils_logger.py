...
def get_logger_tracer(name: str = "vpaserver") -> tuple[logging.Logger, trace.Tracer]:
    """  
...
def get_logger(name: str = "vpaserver") -> logging.Logger:
    """  
    Returns a singleton logger with the specified name in a thread-safe way.
...
def get_tracer(name: str = "vpaserver") -> trace.Tracer:
    """  
    Returns a singleton tracer with the specified name in a thread-safe way.
    
    Args:  
        name (str): The name of the tracer. Defaults to "vpaserver".
    Returns:  
        trace.Tracer: Configured tracer instance.  
    """
    # Get the tracer by name (logging module maintains a registry of loggers)
    name = app_logger_name
    tracer = trace.get_tracer(app_logger_name)
    # Set the log level
    return tracer
